...
class OpenSoC_CMeshTester_Random_VarInjRate(c: OpenSoC_CMesh[Flit], parms: Parameters, rate: Double) extends Tester(c) {
...
//	implicit def int(x: Int): BigInt = x
  //	implicit def int(x: Bits): BigInt = x.litValue()
	...
	val iterationCount		= 32 //if this number gets too high (above 64 for a 2x2, (max packet ID  = 256, so 256 / 4 = 64) in the current test implementation we will overflow packetID and have false errors based on aliasing in the packetID map
	val packetLength		= 3
  ...
  for(iter <- 0 until iterationCount){
	...
	val totalCycleCount = 200*iterationCount //500
  ...
  while((iterationCountPerPort.min < iterationCount) && (cycleCount < totalCycleCount)){
	for(port <- 0 until c.numPorts){
		...
		if((packetReady > 0) && (iterationCountPerPort(port) < iterationCount) && (readyForInj || (wonCoinToss(port) > 0) || (packetIndexPerPort(port) > 0))){
...
/*
	...
	poke(c.io.ports(1).in.packetValid, 1)
	poke(c.io.ports(1).in.packet, myHeadFlit)
	step(1)
	poke(c.io.ports(1).in.packetValid, 1)
	poke(c.io.ports(1).in.packet, myBodyFlit)
	step(1)
	poke(c.io.ports(1).in.packet, zeroFlit)
	poke(c.io.ports(1).in.packetValid, 0)
